({
    doInit: function (component) {
        ...
        actionIsCheck.setCallback(this, function (response) {
            ...
            if (state === 'SUCCESS' && component.isValid()) {
                ...
                console.log('++++32 ' + responseValue.includes('Quote'));
                if (responseValue.includes('Quote')) {
    ...
    fetchStatusPicklist : function(component){
        ...
        action.setParams({
            'objectName': component.get("v.Quote"),
            'field_apiname': component.get("v.Status"),
            'nullRequired': false
        });
        action.setCallback(this, function(a) {
            var state = a.getState();
            console.log('statequote____', state);
            if (state === "SUCCESS"){
                ...
                console.log('qqqqq' , a.getError());
                var errors = a.getError();
                if (errors){
                    if(errors[0] && errors[0].message){
                        console.log('e' ,errors[0].message);
                        if(errors[0].message == 'Quote not Present') {
                            component.set('v.isQuoteEnabled',false);
                        }
                    }
                }
            }
    ...
    saveOrderMapping: function (component) {
        if(component.find('childlwc') != undefined){
            ...
            var quoteMapperList = component.get('v.orderFieldMapperList');
            for( let i=0; i< statusValues.length; i++) {
                let index = quoteMapperList.findIndex(o => o.Id === statusValues[i].Id);
                quoteMapperList[index] = statusValues[i];
            }
        }
        console.log('quoteMapperList---',quoteMapperList); 
